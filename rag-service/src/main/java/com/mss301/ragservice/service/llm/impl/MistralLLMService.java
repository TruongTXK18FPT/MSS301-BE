...
public class MistralLLMService implements LLMService {
    ...
    public String generateResponse(String query, String context, ResponseMode mode) {
        ...
        try {
            ...
            String response = mistralChatClient.prompt()
                    .user(prompt)
                    .call()
                    .content();

            log.info("Mistral AI response generated successfully. Length: {} characters",
                    response.length());
            ...
            throw new RagServiceException(
                    "Failed to generate response from Mistral AI: " + e.getMessage(),
                    e
            );
        }
    }
